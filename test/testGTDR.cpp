...
class GTDRTerminateCheckTelegram : public TerminateCheckTelegram
{
public:

    GTDRTerminateCheckTelegram(std::string breakStage, std::size_t breakProgress) : TerminateCheckTelegram(breakStage, breakProgress) {}
...
//     algorithm.setBandwidthCriterionType(GTDR::CV);
//     algorithm.setHasHatMatrix(true);
//     REQUIRE_NOTHROW(algorithm.fit());
...
TEST_CASE("GTDR: basic flow with bandwidth optimization (AIC)")
{
    ...
    algorithm.setSpatialWeights(spatials);
    algorithm.setEnableBandwidthOptimize(true);
    algorithm.setBandwidthCriterionType(GTDR::AIC);
    algorithm.setHasHatMatrix(true);
    REQUIRE_NOTHROW(algorithm.fit());
...
#ifdef ENABLE_OPENMP
TEST_CASE("GTDR: basic flow (multithread)")
{
    mat londonhp100_coord, londonhp100_data;
    vector<string> londonhp100_fields;
    if (!read_londonhp100(londonhp100_coord, londonhp100_data, londonhp100_fields))
...
// #ifdef ENABLE_OPENMP
// TEST_CASE("GTDR: basic flow with bandwidth optimization (CV, multithread)")
// {
//     mat londonhp100_coord, londonhp100_data;
//     vector<string> londonhp100_fields;
//     if (!read_londonhp100(londonhp100_coord, londonhp100_data, londonhp100_fields))
...
TEST_CASE("GTDR: cancel")
{
    ...
    SECTION("fit | CV Bandwidth | serial")
    {
        for (auto &&stage : fit_stages)
        {
            ...
            auto telegram = make_unique<GTDRTerminateCheckTelegram>(stage.first, stage.second);
            GTDR algorithm;
