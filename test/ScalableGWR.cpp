...
TEST_CASE("ScalableGWR: basic flow")
{
    ...
    REQUIRE_THAT(diagnostic.AIC, Catch::WithinAbs(2454.93832121, 1e-6));
    REQUIRE_THAT(diagnostic.AICc, Catch::WithinAbs(2548.83573789, 1e-6));
    REQUIRE_THAT(diagnostic.RSquare, Catch::WithinAbs(0.7418439605, 1e-6));
    REQUIRE_THAT(diagnostic.RSquareAdjust, Catch::WithinAbs(0.7730231604, 1e-6));
...
TEST_CASE("ScalableGWR:  bandwidth  of with AIC")
{
    ...
    GwmRegressionDiagnostic diagnostic = algorithm.diagnostic();
    REQUIRE_THAT(diagnostic.AIC, Catch::WithinAbs(2437.3887978, 1e-6));
    REQUIRE_THAT(diagnostic.AICc, Catch::WithinAbs(2445.49861419, 1e-6));
    REQUIRE_THAT(diagnostic.RSquare, Catch::WithinAbs(0.6979320133, 1e-6));
    REQUIRE_THAT(diagnostic.RSquareAdjust, Catch::WithinAbs(0.7211781422, 1e-6));
