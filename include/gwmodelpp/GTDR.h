...
{
public:
    typedef arma::mat (GTDR::*PredictCalculator)(const arma::mat&, const arma::mat&, const arma::vec&); //!< \~english Calculator to predict \~chinese 用于预测的函数

    typedef arma::mat (GTDR::*FitCalculator)(const arma::mat&, const arma::vec&, arma::mat&, arma::vec&, arma::vec&, arma::mat&);   //!< \~english Calculator to fit \~chinese 用于拟合的函数
...
public:
    ...
    /**
     * @brief \~english Construct a new GTDR object. \~chinese 构造一个新的 GTDR 对象。
     * 
     */
    GTDR() {}
    ...
    /**
     * @brief \~english Destroy the GTDR object. \~chinese 销毁 GTDR 对象。
     * 
     */
    virtual ~GTDR() {}
...
private:
    ...
    PredictCalculator mPredictFunction = &GTDR::predictSerial;  //!< \~english Calculator to predict \~chinese 用于预测的函数
    FitCalculator mFitFunction = &GTDR::fitSerial;              //!< \~english Calculator to fit \~chinese 用于拟合的函数
    ...
    BandwidthCriterionCalculator mBandwidthCriterionFunction = &GTDR::bandwidthCriterionCVSerial;   //!< \~english Calculator to get criterion for given bandwidth value \~chinese 用于根据给定带宽值计算指标值的函数
    ...
    IndepVarCriterionCalculator mIndepVarCriterionFunction = &GTDR::indepVarCriterionSerial;    //!< \~english Calculator to get criterion for given independent variable combination \~chinese 用于根据给定变量组合计算指标值的函数
...
public:
    ...
    /**
     * @brief \~english Construct a new GTDRBandwidthOptimizer object. \~chinese 构造一个新的 GTDRBandwidthOptimizer 对象。 
     * 
     ...
     * 
     * @param instance \~english A GTDR instance \~chinese 一个 GTDR 实例
